...
public class SearchItemList implements Serializable
{
   ...
   public ArrayList<Service> getAllServices()
   {
      ...
      {
         if (searchItems.get(i) instanceof Service)
         {
            s.add((Service) searchItems.get(i));
         }
      }
      return s;
   }
   ...
   public ArrayList<Facility> getAllFacilities()
   {
      ...
      for (int i = 0; i < searchItems.size(); i++)
      {
         if (searchItems.get(i) instanceof Facility)
         {
            f.add((Facility) searchItems.get(i));
         }
      }
      return f;
   }
   ...
   public ArrayList<Service> getServiceBySearch(String info)
   {
      ...
      for (int i = 0; i < searchItems.size(); i++)
      {
         if (searchItems.get(i) instanceof Service && (searchItems.get(i).getTitle().toLowerCase().contains(info.toLowerCase()) || searchItems.get(i).getPostcode().contains(info)))
         {
            filteredServices.add((Service) searchItems.get(i));
         }
      }
      return filteredServices;
   }
